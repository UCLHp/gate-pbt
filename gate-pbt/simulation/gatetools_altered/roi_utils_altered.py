...
class region_of_interest_complex(object):
    ...
    def get_mask_complex(self, img, zrange=None, corrected=True, pbar=None):
        ...
        x_coords = np.linspace(orig[0], orig[0] + spacing[0] * size[0], size[0])
        y_coords = np.linspace(orig[1], orig[1] + spacing[1] * size[1], size[1])
        z_coords = np.linspace(orig[2], orig[2] + spacing[2] * size[2], size[2])
        ...
        mask_array = np.zeros((size[2], size[1], size[0]), dtype=np.float32 if corrected else np.uint8)
         ...
         # Loop over z slices in your ROI
        for z, layer in self.contour_layers.items():
            

            # Extract inclusion contours
            inclusion_contours = layer.inclusion

            slice_index = int((z - orig[2]) // spacing[2])
            ...
            for contour in inclusion_contours:
                # Get the vertices of the inclusion contour
                vertices = contour.vertices  # Shape: (n_points, 2)
                x = vertices[:, 0]
                y = vertices[:, 1]
            
                # we want to iterate over the x and y coordinates of the contour
                # and check if they are within the bounds of x and y
                ...
                # Reshape the inside array back to the shape of slice_2d
                inside_2d = inside.reshape(slice_2d.shape)
                
                # Update slice_2d: set elements inside the polygon to 1
                slice_2d[inside_2d] = 1

            mask_array[slice_index] = slice_2d
        ...
        for iz, z in enumerate(z_coords):
            ...
            if distance[0] <= tolerance:
                ...
                contour_z = contour_z_values[idx[0]]
                layer = self.contour_layers[contour_z]

                # Convert image x, y coordinates to physical space
                physical_x = orig[0] + spacing[0] * np.arange(size[0])
                physical_y = orig[1] + spacing[1] * np.arange(size[1])
                x_grid, y_grid = np.meshgrid(physical_x, physical_y, indexing='ij')
                ...
                for contour in layer.inclusion:
                    inclusion_points.extend(contour.vertices)  # Collect inclusion points

                exclusion_points = []
                for contour in layer.exclusion:
                    exclusion_points.extend(contour.vertices)  # Collect exclusion points

                # Convert to numpy arrays for easier handling
                inclusion_points = np.array(inclusion_points)
                exclusion_points = np.array(exclusion_points)
                
                

              
                # Compute the mask for the current z slice
                flat_mask = layer.contains_points(xy_flat)


                if corrected:
                    # Apply corrections if needed (e.g., partial overlaps)
                    flat_mask = flat_mask.astype(np.float32)
                else:
                    flat_mask = flat_mask.astype(np.uint8)

                # Reshape and assign the mask slice
                mask_slice = flat_mask.reshape((size[1], size[0]))  # Shape: (size_y, size_x)
                mask_array[iz, :, :] = mask_slice
                ...
                # No matching contour layer for this z-slice
                continue
        '''
