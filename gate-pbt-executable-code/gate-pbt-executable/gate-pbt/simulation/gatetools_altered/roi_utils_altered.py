...
class contour_layer(object):
    ...
    def contains_points(self, xycoords, ring_mode=False):
        Ncoords = len(xycoords)
        assert(xycoords.shape == (Ncoords,2))
        flatmask = np.zeros(len(xycoords),dtype=bool)
        
        if ring_mode:
            # More complex logic for ring-like containment
            for q in self.inclusion:
                # Check points near the contour boundary
                boundary_points = q.vertices  # or generate points along the contour
                flatmask |= q.contains_points(boundary_points)
        else:
            # Existing logic
            for q in self.inclusion:
                flatmask |= q.contains_points(xycoords)
...
class region_of_interest_complex(object):
    def __init__(self, ds=None, roi_id=None, verbose=False, contours_list=None):
        ...
        self.contour_layers = {}  # Changed to a dictionary mapping z-values to contour_layer objects
        self.zlist = set()
        self.maskparameters = []
        self.masklist = []
        ...
        for contour in roi.ContourSequence:
            ...
            zvalues = np.unique(points[:, 2])

            # Handle contours at multiple z-values
            for zvalue in zvalues:
                slice_points = points[points[:, 2] == zvalue]  # Keep XYZ points
                if zvalue not in self.contour_layers:
                    self.contour_layers[zvalue] = contour_layer(z=zvalue)

                self.contour_layers[zvalue].add_contour(slice_points, ref)
                self.zlist.add(zvalue)
                self.bb.should_contain_all(points[points[:, 2] == zvalue])

        self.zlist = sorted(self.zlist)
        if verbose:
            logger.info(
                "ROI {}={} has {} points on {} contours with z range [{}, {}]".format(
                    self.roinr,
                    self.roiname,
                    self.npoints_total,
                    self.ncontours,
        ...
        for layer in self.contour_layers.values():
            layer.check()

    def have_mask(self):
        return True  # Allow mask creation even with irregular z-values
    

    def get_mask_complex(self, img, zrange=None, corrected=True, pbar=None):
        """
        Compute a mask for the ROI on the given image.
        """
        if not self.have_mask():
            logger.warn("Masking not supported for this ROI")
            return None

        print("[DEBUG] Starting get_mask_complex")
        
        # Get image dimensions and spacings
        spacing = img.GetSpacing()  # (spacing_x, spacing_y, spacing_z)
        size = img.GetLargestPossibleRegion().GetSize()  # (size_x, size_y, size_z)
        orig = img.GetOrigin()  # (origin_x, origin_y, origin_z)

        print(f"[DEBUG] Image size: {size}")
        print(f"[DEBUG] Image spacing: {spacing}")
        print(f"[DEBUG] Image origin: {orig}")

        # Prepare coordinate axes - Fix: use (size - 1) or endpoint=False
        x_coords = np.linspace(orig[0], orig[0] + spacing[0] * (size[0] - 1), size[0])
        y_coords = np.linspace(orig[1], orig[1] + spacing[1] * (size[1] - 1), size[1])
        z_coords = np.linspace(orig[2], orig[2] + spacing[2] * (size[2] - 1), size[2])
        
        print(f"[DEBUG] X coords range: [{x_coords[0]}, {x_coords[-1]}], len={len(x_coords)}")
        print(f"[DEBUG] Y coords range: [{y_coords[0]}, {y_coords[-1]}], len={len(y_coords)}")
        print(f"[DEBUG] Z coords range: [{z_coords[0]}, {z_coords[-1]}], len={len(z_coords)}")
        
        tolerance = spacing[2] / 2.0  # Half of the slice thickness in z-direction
        print(f"[DEBUG] Tolerance: {tolerance}")

        # Build a KD-Tree of contour z-values
        contour_z_values = np.array(self.zlist)
        print(f"[DEBUG] Contour z-values: {contour_z_values}")
        print(f"[DEBUG] Number of contour layers: {len(self.contour_layers)}")
        
        contour_z_tree = cKDTree(contour_z_values[:, np.newaxis])

        # Initialize the mask array
        mask_array = np.zeros((size[2], size[1], size[0]), dtype=np.float32 if corrected else np.uint8)
        print(f"[DEBUG] Mask array shape: {mask_array.shape}")
        
        # Create meshgrid for x and y coordinates ONCE (outside loop)
        # Use indexing='xy' to match standard image conventions
        X, Y = np.meshgrid(x_coords, y_coords, indexing='xy')
        points = np.column_stack((X.ravel(), Y.ravel()))
        print(f"[DEBUG] Meshgrid X shape: {X.shape}, Y shape: {Y.shape}")
        print(f"[DEBUG] Points shape: {points.shape}")
        
        slices_processed = 0
        slices_with_contours = 0
        
        # Loop over IMAGE z slices (not contour z values)
        for iz in range(size[2]):
            z = z_coords[iz]
            
            #if iz % 10 == 0:  # Print every 10th slice to avoid spam
            #    print(f"[DEBUG] Processing slice {iz}/{size[2]}, z={z}")
            
            # Find the closest contour z-value within the tolerance
            # FIX: Query returns scalar when input is 1D
            distance, idx = contour_z_tree.query([z], distance_upper_bound=tolerance)
            
            #print(f"[DEBUG] Slice {iz}: distance type={type(distance)}, distance={distance}")
            #print(f"[DEBUG] Slice {iz}: idx type={type(idx)}, idx={idx}")
            ...
            if np.isscalar(distance):
                ...
                dist_val = distance[0] if len(distance) > 0 else np.inf
                idx_val = idx[0] if len(idx) > 0 else -1
            
            #print(f"[DEBUG] Slice {iz}: dist_val={dist_val}, idx_val={idx_val}, tolerance={tolerance}")
            
            if dist_val <= tolerance and idx_val >= 0:
                slices_with_contours += 1
                
                # Get the contour layer for the matching z-value
                contour_z = contour_z_values[idx_val]
                #print(f"[DEBUG] Slice {iz}: Found matching contour at z={contour_z}")
                
                layer = self.contour_layers[contour_z]
                #print(f"[DEBUG] Slice {iz}: Layer has {len(layer.inclusion)} inclusion contours, {len(layer.exclusion)} exclusion contours")
                
                # Initialize slice mask
                slice_2d = np.zeros((size[1], size[0]), dtype=np.float32 if corrected else np.uint8)
                
                # Process inclusion contours
                for ic, contour in enumerate(layer.inclusion):
                    vertices = contour.vertices  # Shape: (n_points, 2)
                    #print(f"[DEBUG] Slice {iz}, Inclusion contour {ic}: {len(vertices)} vertices")
                    #print(f"[DEBUG] Vertices range: X[{vertices[:, 0].min()}, {vertices[:, 0].max()}], Y[{vertices[:, 1].min()}, {vertices[:, 1].max()}]")
                    
                    # Create the polygon path
                    polygon = vertices  # Already in (x, y) format
                    path = Path(polygon)
                    
                    # Determine which points are inside the polygon
                    inside = path.contains_points(points)
                    #print(f"[DEBUG] Inside shape: {inside.shape}, dtype: {inside.dtype}")
                    
                    # Reshape - must match meshgrid shape
                    inside_2d = inside.reshape(Y.shape)  # (size[1], size[0])
                    #print(f"[DEBUG] Inside_2d shape: {inside_2d.shape}, sum: {inside_2d.sum()}")
                    
                    # Update slice_2d: set elements inside the polygon to 1
                    slice_2d[inside_2d] = 1
                
                # Process exclusion contours
                for ec, contour in enumerate(layer.exclusion):
                    vertices = contour.vertices
                    #print(f"[DEBUG] Slice {iz}, Exclusion contour {ec}: {len(vertices)} vertices")
                    
                    polygon = vertices
                    path = Path(polygon)
                    inside = path.contains_points(points)
                    inside_2d = inside.reshape(Y.shape)
                    
                    # Remove excluded regions
                    slice_2d[inside_2d] = 0
                    #print(f"[DEBUG] Exclusion applied, slice_2d sum: {slice_2d.sum()}")
                
                # Assign to mask array
                mask_array[iz] = slice_2d
                #print(f"[DEBUG] Slice {iz}: Assigned to mask_array, non-zero count: {(slice_2d > 0).sum()}")
                
                if pbar:
                    ncorners_in = sum(len(contour.vertices) for contour in layer.inclusion)
                    ncorners_ex = sum(len(contour.vertices) for contour in layer.exclusion)
        ...
        print(f"[DEBUG] Processed {slices_processed} slices, {slices_with_contours} had contours")
        print(f"[DEBUG] Total non-zero voxels in mask: {(mask_array > 0).sum()}")

        # Convert mask array back to ITK image
        roimask = itk.GetImageFromArray(mask_array)
        roimask.SetSpacing(img.GetSpacing())
        roimask.SetOrigin(img.GetOrigin())
        roimask.SetDirection(img.GetDirection())

        print("[DEBUG] Mask creation complete")
        return roimask
