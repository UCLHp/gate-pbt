...
def full_analysis(outputdir):
    ...
    for field in fieldnames:
        ...
        if dose in [basename(f) for f in mergedfiles]:
            ...
            output_path = os.path.join(outputdir, f'uncertainty_metrics_{field}.txt')
...
def find_output_folders(main_folder):
    """Process each subfolder in the main folder, and for each one, look for an 'output' folder
    If not found, look one level deeper in that subfolder's subfolders"""
    output_folders = []
    
    try:
        # Get all immediate subfolders in the main folder
        level1_subfolders = [join(main_folder, d) for d in os.listdir(main_folder) if isdir(join(main_folder, d))]
        
        # Process each level 1 subfolder
        for subfolder in level1_subfolders:
            print(f"Checking subfolder: {subfolder}")
            
            # Check if there's an 'output' folder directly in this subfolder
            level2_dirs = [d for d in os.listdir(subfolder) if isdir(join(subfolder, d))]
            
            output_found = False
            for dir_name in level2_dirs:
                if dir_name.lower() == 'output':
                    output_folder = join(subfolder, dir_name)
                    output_folders.append(output_folder)
                    print(f"  Found output folder: {output_folder}")
                    output_found = True
                    break
            
            # If no output folder found at level 2, check level 3 (subfolders of this subfolder)
            if not output_found:
                print(f"  No direct output folder in {subfolder}, checking one level deeper")
                for level2_dir in level2_dirs:
                    level2_path = join(subfolder, level2_dir)
                    try:
                        level3_dirs = [d for d in os.listdir(level2_path) if isdir(join(level2_path, d))]
                        
                        for dir_name in level3_dirs:
                            if dir_name.lower() == 'output':
                                output_folder = join(level2_path, dir_name)
                                output_folders.append(output_folder)
                                print(f"  Found output folder at level 3: {output_folder}")
                                break
                    except (PermissionError, FileNotFoundError) as e:
                        print(f"  Error accessing {level2_path}: {e}")
                        
    except (PermissionError, FileNotFoundError) as e:
        print(f"Error accessing main folder {main_folder}: {e}")
    
    return output_folders

if __name__ == "__main__":
    
    # Select main directory to search for Gate output folders
    msg = "Select main directory containing patient folders"
    ...
    main_dir = easygui.diropenbox()
    print(f"Selected main directory: {main_dir}")
    
    # Find all 'output' folders
    print("\nSearching for output folders...")
    output_folders = find_output_folders(main_dir)
    
    if not output_folders:
        easygui.msgbox("No 'output' folders found in the selected directory structure.", "No Output Folders")
        sys.exit(0)
    
    print(f"\nFound {len(output_folders)} output folders to process.")
    summary_msg = "\n".join([f"{i+1}. {folder}" for i, folder in enumerate(output_folders)])
    if easygui.ccbox(f"Found {len(output_folders)} output folders:\n\n{summary_msg}\n\nProceed with analysis?", "Confirm Processing"):
        pass
    else:
        sys.exit(0)
    ...
    for folder_index, output_folder in enumerate(output_folders):
        print(f"\n\n==== Processing folder {folder_index+1}/{len(output_folders)} ====")
        print(f"Output folder: {output_folder}")
        
        try:
            full_analysis(output_folder)
            print(f"Completed analysis for {output_folder}")
        except Exception as e:
            print(f"Error processing {output_folder}: {e}")
            import traceback
            traceback.print_exc()
            
            # Ask if user wants to continue with next folder
            if len(output_folders) > folder_index + 1:
                cont_msg = f"Error processing folder. Continue with next folder?"
                if not easygui.ccbox(cont_msg, "Continue Processing?"):
                    break
