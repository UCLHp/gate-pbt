...
def full_analysis( outputdir ):
    ...
    output_path_gamma = os.path.join(outputdir, 'gamma_values.txt')
    open(output_path_gamma, 'w').close()  # Creates empty file
    ...
    for field in fieldnames:
        ...
        tps_dose = overrides.set_external_dose_zero( tps_dose, struct_file, "zSurface" )    ## hard-coded zSurface?
        ...
        if dose in [basename(f) for f in mergedfiles]:
            ...
            gamma_img, gamma_img_local = gamma.gamma_image(  d2wimg, tps_dose, 3, 3 )
            itk.imwrite(gamma_img, join(outputdir, field+"_Gamma_33.mhd") )

            itk.imwrite(gamma_img_local, join(outputdir, field+"_Gamma_33_local.mhd") )

            pass_rate_33 = gamma.get_pass_rate( gamma_img )
            pass_rate_33_local = gamma.get_pass_rate( gamma_img_local )

            print("   *** Gamma pass rate @ 3%/3mm GLOBAL = {}%".format( round(pass_rate_33,2) ))
            print("   *** Gamma pass rate @ 3%/3mm LOCAL = {}%".format( round(pass_rate_33_local,2) ))
            ...
            print("  Converting gamma local image to dicom")
            gamma_dcm_local = join(outputdir, field+"_Gamma_33_local.dcm")
            ...
            gamma_img_22, gamma_img_22_local = gamma.gamma_image(  d2wimg, tps_dose, 2, 2 )
            
            itk.imwrite(gamma_img_22, join(outputdir, field+"_Gamma_22.mhd") )
            itk.imwrite(gamma_img_22_local, join(outputdir, field+"_Gamma_22_local.mhd") )

            pass_rate_22 = gamma.get_pass_rate( gamma_img_22 )
            print("   *** Gamma pass rate @ 2%/2mm GLOBAL = {}%".format( round(pass_rate_22,2) ))
            pass_rate_22_local = gamma.get_pass_rate( gamma_img_22_local )
            print("   *** Gamma pass rate @ 2%/2mm LOCAL = {}%".format( round(pass_rate_22_local,2) ))
...
def get_output_directory():
    """Get OUTPUT directory path from command line argument or GUI."""
    debug_print(f"\n--- get_output_directory called ---")
    debug_print(f"sys.argv: {sys.argv}")
    debug_print(f"Current working directory: {os.getcwd()}")
    
    # Get path from argument or GUI
    if len(sys.argv) > 1:
        output_dir = _get_path_from_argument(sys.argv[1])
    else:
        debug_print("No output directory provided as argument -> Opening GUI")
        output_dir = _get_output_path_from_gui()
    
    # Validate the path exists
    if not output_dir or not Path(output_dir).exists():
        _handle_invalid_path(output_dir)
    
    debug_print(f"Using output directory: {output_dir}")
...
def _get_path_from_argument(path_input):
    ...
    output_dir = Path(path_input)
    debug_print(f"As Path object: {output_dir}")
    debug_print(f"Is absolute: {output_dir.is_absolute()}")
    
    # Resolve relative paths from CWD
    if not output_dir.is_absolute():
        output_dir = Path.cwd() / output_dir
        debug_print(f"Resolved from CWD: {output_dir}")
    
    output_dir = output_dir.resolve()
    debug_print(f"Final resolved path: {output_dir}")
    debug_print(f"Path exists: {output_dir.exists()}")
    
    return output_dir


def _get_output_path_from_gui():
    """Open GUI to select output directory."""
...
def _handle_invalid_path(output_dir):
    """Handle invalid or non-existent paths with helpful error messages."""
    debug_print(f"✗ ERROR: Output path does not exist!")
    debug_print(f"  Provided: {output_dir}")
